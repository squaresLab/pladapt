...
SimulationResults Simulation::run(const SimulationParams& simParams, const Params& params,
  ...
  // load monitoring info from filesystem
  ...
  currentConfig = adaptMgr.convertToDiscreteConfiguration(monitoringInfo);
	...
	//Route senseRoute(monitoringInfo.position, monitoringInfo.directionX, monitoringInfo.directionY, params.adaptationManager.HORIZON);
        ...
        //cout << "Standby monitoring info...\n";
            ...
            double threatP = getMean(envThreatMonitor.getBetaDistribution(position));
            double targetP = getMean(envTargetMonitor.getBetaDistribution(position));
            /*
            for (int i = 0; i < senseRoute.size(); i++){
                //cout << senseRoute[i] << " "<<threatP<<";"<<targetP<<" ";
                cout << "threat: " << pThreatSim->getProbabilityOfDestruction(currentConfig);
                cout << " target: " << pTargetSensor->getProbabilityOfDetection(currentConfig);
            }
            cout << "\n";
            */
