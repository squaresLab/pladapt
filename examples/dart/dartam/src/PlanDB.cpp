...
namespace dart{
	...
	// TODO: Convert this to Dart states
	bool PlanDB::populate_state_obj(const DartConfiguration* config,
		...
		State& state) {
			...
			//TODO: Adapt this to new probability [0-1] range

			//TODO: Confirm that these are the correct components for target and threat
			...
			for (unsigned s = 0; s < oldPredictions->getNumberOfStates(); s++) {
				...
				if((abs(threatProb - currentThreatProb) + abs(targetProb - currentTargetProb)) < tolerance) {
					state.env_state = s;
					break;
				}
